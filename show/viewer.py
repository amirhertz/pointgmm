...
def vertices(mesh, plot):
    vs, _, _ = mesh
    plot[0].scatter(vs[:, 0], vs[:, 1], vs[:, 2])
    return plot
...
def plot_mesh(mesh, *whats, show, plot=None):
...
def parse_points(points_file, scale_by):
    vs = []

    def fix_vertices():
        nonlocal vs, scale_by
        vs = V(vs)
        if type(points_file) is str:
            z = vs[:, 2].copy()
            vs[:, 2] = vs[:, 1]
            vs[:, 1] = z
        max_range = 0
    ...
    if type(points_file) is str:
        with open(points_file) as f:
            for line in f:
                line = line.strip()
                splitted_line = line.split()
                if not splitted_line or len(splitted_line) != 3:
                    continue
                vs.append([float(v) for v in splitted_line])

    elif str(type(points_file)) == "<class 'torch.Tensor'>":
        vs = points_file.cpu().numpy()
        # scale_by = 1
    else:
        vs = points_file
        # scale_by = 1
...
def view_points(*files, offset=.2, save_path=''):
    plot = None
    max_x = 0
    scale = 0
    for f_idx, file in enumerate(files):
        points, scale = parse_points(file, scale)
        max_x_current = points[:, 0].max()
        points[:, 0] += max_x + offset
        if f_idx + 1 == len(files):
            if save_path:
                show = save_path
            else:
                show = True
            ...
            show = False
        plot = plot_mesh((points, None, None), vertices, plot=plot, show=show)
        max_x += max_x_current + offset
